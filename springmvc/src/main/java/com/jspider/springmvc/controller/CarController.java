...
public class CarController {
	...
	public String findAllCars(ModelMap modelMap,
			HttpSession httpSession) {
		UserDTO signdInUser=(UserDTO)httpSession.getAttribute("user");
		if (signdInUser!=null) {
			List<CarDTO> cars=carService.findAllCars();
			if (cars!=null) {
				modelMap.addAttribute("car",cars);
			}else {
				modelMap.addAttribute("message","Car Not Available");
			}
			return "cars";
			}else {
				return "sign_in";
...
public String deleteCar(@RequestParam(name="id") int id ,ModelMap modelMap,HttpSession httpSession) {
	...
	if (cars!=null) {
		modelMap.addAttribute("cars",cars);
		modelMap.addAttribute("message","Car Detail Deleted");		
	}else {
		modelMap.addAttribute("cars",cars);
	}
	return "my_cars";	
...
public String updateCar(@RequestParam(name="id") int id,@RequestParam(name="name")String name,
	...
	List<CarDTO> cars=carService.findAllCarsByUser(signedInUser);
	modelMap.addAttribute("cars",cars);
	modelMap.addAttribute("messsage","Car details updated");
	return "my_cars";	
}
